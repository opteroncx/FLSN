...
import numpy as np
import math
from models.modules import upsample_block,Nonlocal_CA,WavePool,WaveUnpool,CARB,FRM
...
class NE(nn.Module):
    def __init__(self, channels=1):
        ...
        features = 64
        # layers = []
        self.head3 = nn.Conv2d(in_channels=channels, out_channels=features, kernel_size=kernel_size, padding=padding, bias=False)
        self.relu = nn.ReLU()
        self.block = CARBN(64)
        self.tail3 = nn.Conv2d(in_channels=features, out_channels=channels, kernel_size=kernel_size, padding=padding, bias=False)
        # self.dncnn = nn.Sequential(*layers)
    def forward(self, x):
        out = self.relu(self.head3(x))
        out = self.block(out)
        out = self.tail3(out)
        return out
...
class Net(nn.Module):
    def __init__(self, n_channels=1):
        ...
        self.inc3 = DoubleConv(2*n_channels, 64)
        self.down2_1 = Down2()
        self.down2_2 = Down2()
        self.down2_3 = Down2()
        # Branches
    ...
    def forward(self, x):
        x0 = self.LE_head(x)
        x1 = self.inc3(torch.cat([x0,x],dim=1))
        b1 = self.branch1(x1)
        b1 = self.scale1(b1)
